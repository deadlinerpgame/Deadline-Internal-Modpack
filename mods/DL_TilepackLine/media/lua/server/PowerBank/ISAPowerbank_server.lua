...
function SPowerbank:fromModData(modData)
    self.on = modData.on
    self.infinite = modData.infinite
...
function SPowerbank:toModData(modData)
    modData.on = self.on
    modData.infinite = self.infinite
...
function SPowerbank:shouldDrain(isoPb)
    if self.infinite == true then return false end
    --print(self.infinite)
...
function SPowerbank:getSprite(updatedCH)
    ...
    elseif self.infinite == true then
        return "d_trash_1_1"
    end
end
