...
local function ToggleAdminPower(player,powerbank,activate)
	local character = getSpecificPlayer(player)
	if luautils.walkAdj(character, powerbank:getSquare(), true) then
		ISTimedActionQueue.add(isa.ToggleAdminPower:new(character, powerbank, activate))
	end
end
...
function UI.OnFillWorldObjectContextMenu(player, context, worldobjects, test)
	...
	if powerbank then
		...
		if isAdmin() or isDebugEnabled() then
			local isInfinite = powerbank:getModData()["infinite"]
			if isInfinite == false then
				ISASubMenu:addOption(getText("ADMIN: Enable Infinite Power"), player, ToggleAdminPower, powerbank, not isInfinite)
			elseif isInfinite == true then
				ISASubMenu:addOption(getText("ADMIN: Disable Infinite Power"), player, ToggleAdminPower, powerbank, not isInfinite)
			end
		end
