...
DiceRangeMarkers.MOVE_RANGE = 5
DiceRangeMarkers.DASH_RANGE = 10
...
local curMove = DiceRangeMarkers.MOVE_RANGE
local curDash = DiceRangeMarkers.DASH_RANGE
local curThrow = DiceRangeMarkers.THROW_RANGE

local function refreshRanges()
	curMove = Core.moveRange and Core.moveRange() or DiceRangeMarkers.MOVE_RANGE
	curDash = curMove * 2
	curThrow = Core.throwRange and Core.throwRange() or DiceRangeMarkers.THROW_RANGE
end

local function lineBandColor(dist, moveOn, gunOn)
	if moveOn and dist <= curMove then
		return COLOR_MOVE
	end
	if moveOn and dist <= curDash then
		return COLOR_DASH
	end
	if gunOn and dist <= curThrow then
		return COLOR_THROW
	end
...
local function onRenderTick()
	...
	refreshRanges()
	if moveOn then
		drawRing(cell, px, py, z, curMove, COLOR_MOVE, 0.65)
		drawRing(cell, px, py, z, curDash, COLOR_DASH, 0.45)
	end
	if gunOn then
		drawRing(cell, px, py, z, curThrow, COLOR_THROW, 0.45)
...
local function onPostRender()
	...
	local sx = isoToScreenX(0, lastTile.x + 0.5, lastTile.y + 0.5, lastTile.z) * zoom
	local sy = isoToScreenY(0, lastTile.x + 0.5, lastTile.y + 0.5, lastTile.z) * zoom
	local offset = 51
