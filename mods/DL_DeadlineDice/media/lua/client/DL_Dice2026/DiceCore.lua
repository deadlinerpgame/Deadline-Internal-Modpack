...
local function dayKey(day)
	return tostring(math.floor(tonumber(day) or 0))
end
...
local vanillaClassify = DiceCore.classifyWeapon
...
function DiceCore.myTraits()
	if myTraits ~= nil then
		return myTraits
	end
	...
	myTraits = DiceTraits.fromPlayer(p)
	return myTraits
end

function DiceCore.clearTraitCache()
	myTraits = nil
end
...
function DiceCore.dashRange()
	return DiceCore.moveRange() * 2
end

function DiceCore.throwRange()
	return DiceTraits.throwRange(DiceCore.myTraits())
end

function DiceCore.heldItem()
	local p = getPlayer and getPlayer() or nil
	return p and p:getPrimaryHandItem() or nil
end

function DiceCore.canThrow()
	local item = DiceCore.heldItem()
	if item == nil then
		return false
	end
	local class = DiceCore.classifyWeapon(item)
	return DiceTraits.canThrowItem(DiceCore.myTraits(), DiceTraits.categoriesOf(item), item:getFullType(), class)
end

function DiceCore.traitModifier(rollId)
	local traits = DiceCore.myTraits()
	if #traits == 0 then
		return nil
	end
	...
	local item = DiceCore.heldItem()
	local class = DiceCore.classifyWeapon(item)
	if rollId == "throw" and class ~= "unarmed" and class ~= "molotov" and class ~= "bomb" then
		class = "thrown"
	end
	...
	return DiceTraits.modifierText(traits, "attack", {
		class = class,
		cats = cats,
		gunRange = DiceTraits.gunRangeOf(item, class),
...
function DiceCore.send(cmd, args)
	...
	elseif DiceServer and DiceServer.handle then
		DiceServer.handle(getPlayer(), cmd, args)
	else
...
local function applyServerCommand(cmd, args)
	...
	elseif cmd == "commitment" then
		local hash = args.hash
		if args.day and type(hash) == "string" then
			local day = dayKey(args.day)
			...
			elseif known ~= hash then
				DiceCore.send("auditReport", { day = tonumber(day), hashConflict = true, known = known, received = hash })
			end
		end
...
function DiceCore.receiveAuditChunk(args)
	...
	if args.done then
		auditJob = { day = auditBuffer.day, seed = auditBuffer.seed, rolls = auditBuffer.rolls, index = 0, bad = 0, details = {} }
		auditBuffer = nil
		local stored = DiceCore.settings.commitments and DiceCore.settings.commitments[dayKey(auditJob.day)] or nil
		auditJob.commit = stored
		if stored and sha256(auditJob.seed) ~= stored then
			DiceCore.send("auditReport", { day = auditJob.day, seedMismatch = true })
			auditJob = nil
		end
	end
end
...
local function stepAudit()
	...
	while j.index < #j.rolls and steps < 20 do
		...
		if v ~= rec.r then
			j.bad = j.bad + 1
			if j.bad <= 5 then
				j.details[#j.details + 1] = { c = rec.c, i = rec.i, r = rec.r, v = v }
			end
		end
	end
	if j.index >= #j.rolls then
		DiceCore.send("auditReport", {
			day = j.day, total = #j.rolls, bad = j.bad, commitMatched = j.commit ~= nil, details = j.details,
		})
		auditJob = nil
	end
end
...
function DiceCore.canRoll(def)
	local s = DiceCore.state
	if def and def.id == "throw" and not DiceCore.canThrow() then
		return false
	end
	if not s.combatId then
		return true
	end
	if s.paused then
		return false
...
function DiceCore.roll(id)
	...
	if not DiceCore.state.combatId then
		local me = getLocalMe()
	...
	if id == "attack" or id == "throw" then
		...
		local class = DiceCore.classifyWeapon(item)
		local ammo = nil
		if id ~= "throw" and GUN_CLASSES[class] then
			ammo = item:getCurrentAmmoCount() or 0
			if ammo <= 0 then
...
function DiceCore.isStaff()
	...
	return level ~= nil and level ~= "None"
end
...
function DiceCore.checkAdminChange()
	local isAdmin = DiceCore.isLocalAdmin()
	if DiceCore.wasAdmin == nil then
		DiceCore.wasAdmin = isAdmin
		return
	end
	if isAdmin ~= DiceCore.wasAdmin then
		DiceCore.wasAdmin = isAdmin
		if DiceCore.rebuildPanel then
			DiceCore.rebuildPanel("toolbar")
		end
		DiceCore.notify("admin")
	end
end
