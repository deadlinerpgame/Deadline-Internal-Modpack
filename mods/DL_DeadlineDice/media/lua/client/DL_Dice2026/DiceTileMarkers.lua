...
local function resolvePos(c)
	...
	if not c.isNpc and c.name and getPlayerFromUsername then
		local p = getPlayerFromUsername(c.name)
		if p then
			return math.floor(p:getX()), math.floor(p:getY()), math.floor(p:getZ())
		end
	end
...
local function onPostRender()
	...
	for i = 1, #markers do
		...
		local x, y, z = resolvePos(m.c)
		if x then
			local sx = isoToScreenX(0, x + 0.5, y + 0.5, z) * zoom
			local sy = isoToScreenY(0, x + 0.5, y + 0.5, z) * zoom
