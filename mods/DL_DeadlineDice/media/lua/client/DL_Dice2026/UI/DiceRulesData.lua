...
DiceRulesData.sections = {
	...
	},
	{
		title = "Actions",
		pages = {
			"OVERVIEW\n\nWhen in dice combat, we speak of 'Actions'. Each turn a player may move 5 tiles and perform one action from the list below:\n\n- Dashing\n- Escaping\n- Surrendering\n- Grappling\n- Disengaging\n- Attacking\n- Reloading\n- Changing weapons",
		},
	},
	{
	...
	},
	{
		...
		pages = {
			...
			"TRAITS THAT CHANGE CRITICALS\n\nWildcard: a roll of 19 or 20 is a critical success, and a roll of 1 or 2 is a critical failure.\n\nEven Keel: you never land a critical success and you never suffer a critical failure.\n\nLucky and Unlucky no longer change critical rolls. Lucky ends status effects one turn sooner, Unlucky makes them last one turn longer.\n\nCRITICAL FAILURE (NATURAL 1)\n\nRolling a natural 1 on an attack = automatic miss.\n\nOpponent does not roll to defend.\n\nThe text will display 'Critical Failure' next to the roll.\n\nCRITICAL FAILURE ON A DEFENCE\n\nRolling a natural 1 when you defend means the attack lands no matter what the totals say, and 1d4 is added to its damage. Wildcard fumbles a defence on a 1 or a 2, Even Keel never fumbles one.",
			...
			"THROWING EXPLOSIVES\n\nIf you critically fail when throwing any explosive, the throw goes wide:\n\n- The system rolls 1d3 and the explosive lands that many tiles from your target, in a random direction.\n- It goes off where it lands and is resolved like any other explosive: everyone inside the blast rolls Defend (ranged) against your throw, including you and your own side.\n- Roleplay the bad throw: a slipped grip, a bad bounce, a panicked lob.",
		},
	},
	{
		title = "Surrendering",
	...
	},
	{
		...
		pages = {
			...
			"ESCAPE ROLL\n\n- The escapee rolls the escape dice. It is a flat roll: how far away anyone is standing makes no difference.\n- To succeed, the result must be 18 or higher.\n- Traits and the advantage toggle still count towards the result. Slippery adds 2.\n\nOUTCOME\n\n- Success: The escape is successful, and the escapee must leave the scene. (They may not stick around to watch OOC.)\n- Failure: The attempt fails, and the escapee remains in combat. They may now use their 5 tiles of movement.",
	...
	},
	{
		...
		pages = {
			{
				text = "On your turn, you can move up to 5 tiles. You can move in any direction, horizontal, vertical, or diagonal. By default, every player may move 5 tiles each turn. Featherweight gives you one tile more, Heavyweight one tile less, and the move ring in game always shows your own number.\n\nDASH\n\nYou may use the Dash action to gain another 5 tiles of movement on your turn.\n\n- This counts as an action, you can only do one action each turn.\n- You cannot use the Dash action two turns in a row.",
	...
	},
	{
		...
		pages = {
			"An Attack of Opportunity (AoO) is a special reaction that lets a player make a free attack during another player's turn.\n\nWHEN DOES AN AOO TAKE PLACE?\n\nAn Attack of Opportunity is triggered when:\n\n- A player leaves the melee range (1 tile) of their opponent and wants to move away without using the Disengage action.\n- A player moves directly past (1 tile) an opponent within that opponent's melee range.\n- Only a character with the Spearwall trait may take one, and only while they hold a two handed melee weapon. Nobody else gets attacks of opportunity.",
			"HOW DOES IT WORK?\n\nThe dice system handles this for you. While it is your turn, a Spearwall fighter holding a two handed melee weapon in melee range of you carries the covering icon and you carry the threatened icon. Step out of their range and the attack of opportunity is rolled at once.\n\n- The opponent makes an attack roll (melee only).\n- The moving player makes a Defend (Close) roll.\n- Damage is applied automatically. The mover may still complete their movement.\n- Each opponent may only take one attack of opportunity per turn of yours.\n- Press Disengage before you move to avoid them entirely.",
...
local function traitSection()
	local defs = {}
	for i = 1, #DiceTraits.order do
		local def = DiceTraits.defs[DiceTraits.order[i]]
		if def then
			defs[#defs + 1] = def
		end
	end
	...
	local pages = {
		"TRAIT EFFECTS\n\nThe dice system reads the traits on your character sheet and applies them on its own. Every modifier shows up in the combat log next to the roll it changed.\n\n- Attack, Defend close, Defend ranged, Hide, Notice, Endurance, First aid, Grapple, Escape and Initiative rolls take their modifiers automatically.\n- Weapon traits only count while you hold that kind of weapon. Gun traits count the gun class you are firing.\n- Crit traits change which rolls count as a critical success or failure.\n- Movement traits change your move and dash rings.",
	}
	local lines = {}
	for i = 1, #defs do
		lines[#lines + 1] = defs[i].name .. ": " .. defs[i].text
		if #lines == 10 or i == #defs then
			pages[#pages + 1] = table.concat(lines, "\n")
			lines = {}
		end
	end
	return { title = "Trait effects", pages = pages }
end

DiceRulesData.sections[#DiceRulesData.sections + 1] = traitSection()
