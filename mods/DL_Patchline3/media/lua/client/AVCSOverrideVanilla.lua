...
function AVCS.addOptionToMenuOutsideVehicle(player, context, vehicle)
	...
	elseif checkResult.permissions == true then
		...
		toolTip.description = getText("Tooltip_AVCS_Expire") .. ": " .. os.date("%d-%b-%y, %H:%M:%S", (checkResult.LastKnownLogonTime + (SandboxVars.AVCS.ClaimTimeout * 60 * 60)))
		end
		option.notAvailable = false
