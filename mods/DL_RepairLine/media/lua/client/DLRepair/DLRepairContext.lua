local function onUseBench(worldobjects, playerNum, bench)
    ...
    if luautils.walkAdj(player, sq, true) then
        ISTimedActionQueue.add(DLRepairOpenBenchAction:new(player, bench))
    end
end

local function onFillWorldContext(playerNum, context, worldobjects, test)
    if test then return end
    local bench = DLRepair.Bench.findIn(worldobjects)
    if not bench then return end
    context:addOption("Use Repair Bench", worldobjects, onUseBench, playerNum, bench)
end
