...
function HuntLineCow.VolumeHandler(dist, range)
    if dist >= range then
        return 0
    elseif dist <= 0 then
        return 1
...
function HuntLineCow.getCowIdle()
	return 'Cow_Idle_'..tostring(ZombRand(1,12))
end

function HuntLineCow.getCowPain()
	return 'Cow_Pain_'..tostring(ZombRand(1,8))
end

function HuntLineCow.getCowStress()
	return 'Cow_Stress_'..tostring(ZombRand(1,7))
end

function HuntLineCow.getCowDeath()
	return 'Cow_Death_'..tostring(ZombRand(1,5))
end


function HuntLineCow.playSFX(zed, sfx, isAttract)
	if not HuntLineCow.isCowAnimal(zed) then return end
	local range = 150
	local pl = getPlayer(); if not pl then return end
	if HuntLineCow.isClosestPl(pl, zed) or pl == zed then
		local sq = zed:getSquare()
		if sq then
			local vol = HuntLineCow.VolumeHandler(HuntLineCow.checkDist(pl, zed), range)
...
function HuntLineCow.getSFX()
	local list = HuntLineCow.sfxZedList
	return list[ZombRand(1, #list)]
end

function HuntLineCow.preSFX(zed)
	if not HuntLineCow.isCowAnimal(zed) then return end
	if zed:getEmitter() then
		local audioName = nil
		local list = HuntLineCow.sfxBanList
		for _, audioName in ipairs(list) do
			if zed:getEmitter():isPlaying(audioName) then
				...
				if HuntLineCow.doRoll(10) then
					--HuntLineCow.playRandSfx(zed)
				end
			end
		end
	end
end
Events.OnZombieUpdate.Remove(HuntLineCow.preSFX)
Events.OnZombieUpdate.Add(HuntLineCow.preSFX)



function HuntLineCow.playRandSfx(zed)
	if not HuntLineCow.isCowAnimal(zed) then return end
	if HuntLineCow.doRoll(5) then
		local toPlay = HuntLineCow.getSFX()
		--zed:getEmitter():playSound(toPlay)
	end
end


function HuntLineCow.OnZombieDead(zed)
	if not HuntLineCow.isCowAnimal(zed) then return end
	HuntLineCow.playSFX(zed, HuntLineCow.getCowDeath())
	local inventory = zed:getInventory()

	local sq = zed:getSquare()
	print("[SERVER] Dropping 'Base.Cow_Dead' at:", sq:getX(), sq:getY(), sq:getZ())
	sq:AddWorldInventoryItem("Base.Cow_Dead", ZombRand(0, 1.5), ZombRand(0, 1.5), 0)

end
Events.OnZombieDead.Remove(HuntLineCow.OnZombieDead)
Events.OnZombieDead.Add(HuntLineCow.OnZombieDead)
