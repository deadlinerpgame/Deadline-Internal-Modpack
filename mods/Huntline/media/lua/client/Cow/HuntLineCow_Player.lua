...
HuntLineCow.skin0 = 'Base.AnimalCow'


-----------------------            ---------------------------
function HuntLineCow.getTargetedZombie(pl)
    local zombies = getCell():getZombieList()
    local targetedZombie = nil
    for i = 0, zombies:size() - 1 do
        local zed = zombies:get(i)
		if HuntLineCow.isTarg(pl, zed) then
            targetedZombie = zed
            break
        end
    end
...
function HuntLineCow.getDistanceBetween(pl, zed)
    local plX, plY = pl:getX(), pl:getY()
    local zedX, zedY = zed:getX(), zed:getY()
    return math.sqrt((plX - zedX) ^ 2 + (plY - zedY) ^ 2)
end

function HuntLineCow.getFov()
    return 90
end

function HuntLineCow.isTarg(pl, targ)
	local fov = HuntLineCow.getFov()
    local plX, plY = pl:getX(), pl:getY()
