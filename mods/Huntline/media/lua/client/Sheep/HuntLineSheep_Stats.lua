...
------------------------               ---------------------------
function HuntLineSheep.doRoll(percent) if percent >= ZombRand(1, 101) then return true end return false end
-----------------------            ---------------------------


function HuntLineSheep.TempMarker(sq, isGood)
...
function HuntLineSheep.TicksSinceSeenZombie()
    local pl = getPlayer()
    return pl.TicksSinceSeenZombie >= 6000
end
function HuntLineSheep.lastSeenZombieTime()
    local pl = getPlayer()
...
function HuntLineSheep.isBiteReact(pl)
	local attacker = nil
...
function HuntLineSheep.setStats(zed)
    if zed then
        ...
        if HuntLineSheep.isSheepAnimal(zed) then
            ...
            if zed:getModData()['SheepAnimal_Init'] == nil then
                ...
                -----------------------            ---------------------------
      --[[           local tSpeed =  SandboxVars.HuntLineSheep.TurnSpeed or 1.5
                if tSpeed then
                    HuntLineSheep.setTurnSpeed(zed, tSpeed)
                end ]]
                -----------------------            ---------------------------
                zed:getModData()['Animal_fType'] = HuntLineSheep.getCorpseFtype(zed)
                zed:setVariable('isSheepAnimal', 'true')
                -----------------------            ---------------------------

                HuntLineSheep.setTag(zed)
                ...
                zed:getModData()['SheepAnimal_Init'] = true
            end
