...
function HuntLinePig.isCrawler(zed)
	return zed:isCrawling()
end

function HuntLinePig.setCrawler(zed)
	...
	local fitNum = tonumber(HuntLinePig.getOutfitCrawlType(zed))
	local crawlType = zed:getCrawlerType()
	if fitNum ~= crawlType and (fitNum == 1 or  fitNum == 2) then
		zed:setCrawlerType(fitNum)
	end
	HuntLinePig.setAnimSpeed(zed, HuntLinePig.getMinAnimSpeed())
end


function HuntLinePig.isWalkable(sq)
...
function HuntLinePig.stopWalking(targ)
	targ:getPathFindBehavior2():reset();
end



function HuntLinePig.isStepped(zed)
...
function HuntLinePig.isShouldStand(zed)
...
function HuntLinePig.setStand(zed)
...
function HuntLinePig.getWalkType(zed)
	return tostring(zed:getVariableString("zombieWalkType"))
end

function HuntLinePig.getWalkNum(zed)
	local walk = HuntLinePig.getWalkType(zed)
	return tonumber(walk:match("%d+"))
end

function HuntLinePig.isSprinter(zed)
	local walk = HuntLinePig.getWalkType(zed)
	if walk then
...
function HuntLinePig.normalize(x, min, max)
    x = math.max(min, math.min(x, max))
    return ((x - min) / (max - min)) * 100
end


function HuntLinePig.setTurnSpeed(zed, int)
...
function HuntLinePig.isMoving(zed)
	...
	if not HuntLinePig.isPigAnimal(zed) then return false end
...
function HuntLinePig.turnAround(targ)
	local x = targ:getX()
	local y = targ:getY()
...
function HuntLinePig.checkWalkableSq()
	local pl = getPlayer()
	...
	if not pl:isOutside() then
		for i=0, rmSqs:size()-1 do
			...
			for _, testSq in ipairs(tab) do
				if testSq  then
					local isGood = false
					if HuntLinePig.isWalkable(testSq) then
						isGood = true
					end
					HuntLinePig.TempMarker(testSq, isGood)
				end
			end
		end
	end
...
function HuntLinePig.moveToXYZ(zed, x, y, z)
