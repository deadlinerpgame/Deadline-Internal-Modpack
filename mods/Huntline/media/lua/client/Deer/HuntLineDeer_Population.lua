...
HuntLineDeer.CorpseCheck = {
    ['Base.AnimalDeer']={["isAnimalCorpse"]=true, ["drop"]="Base.Deer_Dead"},
}
...
function HuntLineDeer.getCorpseFtype(zed)
...
function HuntLineDeer.handleCorpse(corpse)
    if  HuntLineDeer.isDeadAnimal(corpse) or corpse:getModData()['Animal_fType']  then
        local tab = HuntLineDeer.CorpseCheck
...
function HuntLineDeer.CorpseCleaner()
    local rad = 8
        ...
        for yDelta = -rad, rad do
            ...
            if sq then
                ...
                if corpse and instanceof(corpse, "IsoDeadBody") then
                        ...
                        HuntLineDeer.handleCorpse(corpse)
...
Events.EveryOneMinute.Remove(HuntLineDeer.CorpseCleaner)
Events.EveryOneMinute.Add(HuntLineDeer.CorpseCleaner)


function HuntLineDeer.isDeadAnimal(corpse)
	if not corpse or not instanceof(corpse, "IsoDeadBody") then return false end
    if corpse:getModData()['DeerAnimal_Init'] then return true end
    local fit = tostring(corpse:getOutfitName())
    if fit == HuntLineDeer.outfit1 then return true end
    if fit == HuntLineDeer.outfit2 then return true end
...
function HuntLineDeer.getSpawnRandomZedInfo(fit)
...
function HuntLineDeer.doDespawn(zed)
    if zed:isAlive() then
...
function HuntLineDeer.doSpawn(sq, isDown, outfit)
    if sq then
        local x, y, z = sq:getX(), sq:getY(), sq:getZ()
        if isClient() then

            local fit, fChance = HuntLineDeer.getSpawnRandomZedInfo()

            if outfit and outfit ~= '' then
                fit, fChance = HuntLineDeer.getSpawnRandomZedInfo(outfit)
            end
            sendClientCommand('HuntLineDeer', 'doSpawn', {x = x, y = y, z = z, count = 1, fit = fit, fChance = fChance, isDown = isDown})
        else
...
function HuntLineDeer.doLocalSpawn(sq, outfit)
	local pl = getPlayer()
