local FITNESS_XP_LIMIT = 10
local STRENGTH_XP_LIMIT = 10
...
Events.AddXP.Add(function(player, perk, amount)
    ...
    if perk == Perks.Fitness then
        if currentTime - modData.xpCooldown_fitness_timestamp >= 24 then
    ...
    if perk == Perks.Strength then
        ...
        if currentTime - modData.xpCooldown_strength_timestamp >= 24 then
