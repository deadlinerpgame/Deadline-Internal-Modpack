...
function ISTradingUI:update()
    if not getPlayerByOnlineID(self.otherPlayer:getOnlineID()) then
        self.blockingMessage = getText("IGUI_TradingUI_ClosedTrade", self.otherPlayer:getDescriptor():getForename())
    end
    if not self.blockingMessage and (math.abs(getPlayerByOnlineID(self.otherPlayer:getOnlineID()):getX() - self.player:getX()) > 2 or
        ...
        self.blockingMessage2 = getText("IGUI_TradingUI_TooFarAway", self.otherPlayer:getDescriptor():getForename())
        return;
    else
...
function ISTradingUI:prerender()
    ...
    self:drawText(getText("IGUI_TradingUI_HisOffer", self.otherPlayer:getDescriptor():getForename()), self.hisOfferDatas.x, self.hisOfferDatas.y - 32, 1,1,1,1, UIFont.Small);
    ...
    if self.otherSealedOffer then
        self:drawText(getText("IGUI_TradingUI_OtherPlayerSealedOffer", self.otherPlayer:getDescriptor():getForename()), self.sealOffer.x, self.sealOffer.y + self.sealOffer.height + 5, 0.2,1,0.2,1, UIFont.Small);
    end
    z = z + 30;
end
...
ISTradingUI.ReceiveTradeRequest = function (requester)
    local modal = ISModalDialog:new(getCore():getScreenWidth() / 2 - 175,getCore():getScreenHeight() / 2 - 75, 350, 150, getText("IGUI_TradingUI_RequestTrade", requester:getDescriptor():getForename()), true, nil, ISTradingUI.onAnswerTradeRequest);
    modal:initialise()
...
ISTradingUI.AcceptedTrade = function(accepted)
    if ISTradingUI.instance and ISTradingUI.instance:isVisible() then
        ...
        if not accepted then
            ISTradingUI.instance.blockingMessage = getText("IGUI_TradingUI_RefusedTrade", ISTradingUI.instance.otherPlayer:getDescriptor():getForename());
        end
    end
end
...
ISTradingUI.OtherAddNewItem = function(player, item)
    if ISTradingUI.instance and ISTradingUI.instance:isVisible() then
        ...
        ISTradingUI.instance:setHistoryMessage(getText("IGUI_TradingUI_AddedItem", player:getDescriptor():getForename(), item:getName()), true, true, false);
...
ISTradingUI.RemoveItem = function(player, index)
    if ISTradingUI.instance and ISTradingUI.instance:isVisible() then
        ...
        ISTradingUI.instance:setHistoryMessage(getText("IGUI_TradingUI_RemovedItem", player:getDescriptor():getForename(), itemRemoved.item:getName()), true, false, true);
        ISTradingUI.instance.sealOffer.selected[1] = false;
        ISTradingUI.instance.otherSealedOffer = false;
    end
end
...
ISTradingUI.UpdateState = function(player, state)
    if state == ISTradingUI.States.PlayerClosedWindow then
        ...
        elseif ISTradingUI.instance and ISTradingUI.instance:isVisible() and ISTradingUI.instance.otherPlayer == player then
            ISTradingUI.instance.blockingMessage = getText("IGUI_TradingUI_ClosedTrade", player:getDescriptor():getForename());
        end
    end
    if ISTradingUI.instance and ISTradingUI.instance:isVisible() then
        if state == ISTradingUI.States.SealOffer then
            ISTradingUI.instance.otherSealedOffer = true;
            ISTradingUI.instance:setHistoryMessage(getText("IGUI_TradingUI_OtherPlayerSealedOffer", ISTradingUI.instance.otherPlayer:getDescriptor():getForename()), true, false, false);
        end
        if state == ISTradingUI.States.UnSealOffer then
            ISTradingUI.instance.otherSealedOffer = false;
            ISTradingUI.instance:setHistoryMessage(getText("IGUI_TradingUI_OtherPlayerUnSealedOffer", ISTradingUI.instance.otherPlayer:getDescriptor():getForename()), true, false, false);
        end
