...
function TweakWorldObjectContextMenu.tweakContextMenu(player, context, worldobjects, test)
    ...
    if SandboxVars.ServerTweaker.ContextMenuClickedPlayersSelection then
        ...
        for _, option in pairs(options) do
            if option and option.onSelect == ISWorldObjectContextMenu.onTrade then
                option.name = getText('ContextMenu_Trade')
                option.onSelect = nil
                        ...
                        if sq then
                            for i = 0, sq:getMovingObjects():size() - 1 do
                                ...
                                if TweakWorldObjectContextMenu.CanTradeWith(character, clickedPlayer) and not traders[clickedPlayer:getUsername()] then
                                    ...
                                    local optionTrade = tradeSubMenu:addOption(clickedPlayer:getDescriptor():getForename(), worldobjects, ISWorldObjectContextMenu.onTrade, character, clickedPlayer)
                                    if (math.abs(character:getX() - clickedPlayer:getX()) > 2 or math.abs(character:getY() - clickedPlayer:getY()) > 2) then
                                        ...
                                        tooltip.description = getText("ContextMenu_GetCloserToTrade", clickedPlayer:getDescriptor():getForename())
                                ...
                                if TweakWorldObjectContextMenu.CanHealPlayer(character, clickedPlayer) and not patients[clickedPlayer:getUsername()] then
                                    ...
                                    local optionMedicalCheck = medicalCheckSubMenu:addOption(clickedPlayer:getDescriptor():getForename(), worldobjects, ISWorldObjectContextMenu.onMedicalCheck, character, clickedPlayer)
                                    if (math.abs(character:getX() - clickedPlayer:getX()) > 2 or math.abs(character:getY() - clickedPlayer:getY()) > 2) then
                                        local tooltip = ISWorldObjectContextMenu.addToolTip()
                                        optionMedicalCheck.notAvailable = true
                                        tooltip.description = getText("ContextMenu_GetCloser", clickedPlayer:getDescriptor():getForename())
