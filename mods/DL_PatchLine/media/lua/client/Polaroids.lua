...
function Polaroids.PrintPolaroid2(player, item)
    local width = 280
    local height = 180
    local modal = ISTextBox:new(0, 0, width, height, getText("ContextMenu_Polaroids_EnterPhotoUrl"), "", nil, Polaroids.PrintPolaroidSubmit2, player, getSpecificPlayer(player), item)
...
function Polaroids.PrintPolaroidSubmit2(target, button, playerObj, item)
    local player = playerObj:getPlayerNum()
    if button.internal == "OK" then
        playerObj:playSound("PrintPhoto");
        local text = button.parent.entry:getText()
        if text and text ~= "" then
            local polaroid = InventoryItemFactory.CreateItem("Polaroids.PieceOfPaper")
...
function Polaroids.PolaroidItemOptions(player, context, items)
    ...
    for i,item in ipairs(items) do
        if item:getType() == "PolaroidCamera" then
            polaroidCamera = item
            camera = item
        elseif item:getType() == "Polaroid" or item:getType() == "PieceOfPaper" then
            polaroid = item
    ...
    if polaroidCamera then
        context:addOption(getText("ContextMenu_Polaroids_PrintPolaroid"), player, Polaroids.PrintPolaroid, polaroidCamera)
            if isAdmin() or isDebugEnabled() then
                context:addOption(getText("ContextMenu_Polaroids_PrintPolaroidSheetPaper"), player, Polaroids.PrintPolaroid2, polaroidCamera)
            end
    end
    if polaroid then
