...
HuntLineRat.skin0 = 'Base.AnimalRat'


-----------------------            ---------------------------
function HuntLineRat.getTargetedZombie(pl)
    local zombies = getCell():getZombieList()
    local targetedZombie = nil
    for i = 0, zombies:size() - 1 do
        local zed = zombies:get(i)
		if HuntLineRat.isTarg(pl, zed) then
            targetedZombie = zed
            break
        end
    end
...
function HuntLineRat.getDistanceBetween(pl, zed)
    local plX, plY = pl:getX(), pl:getY()
    local zedX, zedY = zed:getX(), zed:getY()
    return math.sqrt((plX - zedX) ^ 2 + (plY - zedY) ^ 2)
end

function HuntLineRat.getFov()
    return 90
end

function HuntLineRat.isTarg(pl, targ)
	local fov = HuntLineRat.getFov()
    local plX, plY = pl:getX(), pl:getY()
