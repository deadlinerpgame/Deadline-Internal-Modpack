...
HuntLineRabbit.skin0 = 'Base.AnimalRabbit'


-----------------------            ---------------------------
function HuntLineRabbit.getTargetedZombie(pl)
    local zombies = getCell():getZombieList()
    local targetedZombie = nil
    for i = 0, zombies:size() - 1 do
        local zed = zombies:get(i)
		if HuntLineRabbit.isTarg(pl, zed) then
            targetedZombie = zed
            break
        end
    end
...
function HuntLineRabbit.getDistanceBetween(pl, zed)
    local plX, plY = pl:getX(), pl:getY()
    local zedX, zedY = zed:getX(), zed:getY()
    return math.sqrt((plX - zedX) ^ 2 + (plY - zedY) ^ 2)
end

function HuntLineRabbit.getFov()
    return 90
end

function HuntLineRabbit.isTarg(pl, targ)
	local fov = HuntLineRabbit.getFov()
    local plX, plY = pl:getX(), pl:getY()
