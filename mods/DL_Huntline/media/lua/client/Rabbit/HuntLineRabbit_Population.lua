...
HuntLineRabbit.CorpseCheck = {
    ['Base.AnimalRabbit']={["isAnimalCorpse"]=true, ["drop"]="Base.Rabbit_Dead"},
}
...
function HuntLineRabbit.getCorpseFtype(zed)
...
function HuntLineRabbit.handleCorpse(corpse)
    if  HuntLineRabbit.isDeadAnimal(corpse) or corpse:getModData()['Animal_fType']  then
        local tab = HuntLineRabbit.CorpseCheck
...
function HuntLineRabbit.CorpseCleaner()
    local rad = 8
        ...
        for yDelta = -rad, rad do
            ...
            if sq then
                ...
                if corpse and instanceof(corpse, "IsoDeadBody") then
                        ...
                        HuntLineRabbit.handleCorpse(corpse)
...
Events.EveryOneMinute.Remove(HuntLineRabbit.CorpseCleaner)
Events.EveryOneMinute.Add(HuntLineRabbit.CorpseCleaner)


function HuntLineRabbit.isDeadAnimal(corpse)
	if not corpse or not instanceof(corpse, "IsoDeadBody") then return false end
    if corpse:getModData()['RabbitAnimal_Init'] then return true end
    local fit = tostring(corpse:getOutfitName())
    if fit == HuntLineRabbit.outfit1 then return true end
    if fit == HuntLineRabbit.outfit2 then return true end
...
function HuntLineRabbit.getSpawnRandomZedInfo(fit)
...
function HuntLineRabbit.doDespawn(zed)
    if zed:isAlive() then
...
function HuntLineRabbit.doSpawn(sq, isDown, outfit)
    if sq then
        local x, y, z = sq:getX(), sq:getY(), sq:getZ()
        if isClient() then

            local fit, fChance = HuntLineRabbit.getSpawnRandomZedInfo()

            if outfit and outfit ~= '' then
                fit, fChance = HuntLineRabbit.getSpawnRandomZedInfo(outfit)
            end
            sendClientCommand('HuntLineRabbit', 'doSpawn', {x = x, y = y, z = z, count = 1, fit = fit, fChance = fChance, isDown = isDown})
        else
...
function HuntLineRabbit.doLocalSpawn(sq, outfit)
	local pl = getPlayer()
