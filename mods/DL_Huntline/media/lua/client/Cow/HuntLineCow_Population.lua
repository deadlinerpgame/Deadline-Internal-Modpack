...
HuntLineCow.CorpseCheck = {
    ['Base.AnimalCow']={["isAnimalCorpse"]=true, ["drop"]="Base.Cow_Dead"},
}
...
function HuntLineCow.getCorpseFtype(zed)
...
function HuntLineCow.handleCorpse(corpse)
    if  HuntLineCow.isDeadAnimal(corpse) or corpse:getModData()['Animal_fType']  then
        local tab = HuntLineCow.CorpseCheck
...
function HuntLineCow.CorpseCleaner()
    local rad = 8
        ...
        for yDelta = -rad, rad do
            ...
            if sq then
                ...
                if corpse and instanceof(corpse, "IsoDeadBody") then
                        ...
                        HuntLineCow.handleCorpse(corpse)
...
Events.EveryOneMinute.Remove(HuntLineCow.CorpseCleaner)
Events.EveryOneMinute.Add(HuntLineCow.CorpseCleaner)


function HuntLineCow.isDeadAnimal(corpse)
	if not corpse or not instanceof(corpse, "IsoDeadBody") then return false end
    if corpse:getModData()['CowAnimal_Init'] then return true end
    local fit = tostring(corpse:getOutfitName())
    if fit == HuntLineCow.outfit1 then return true end
    if fit == HuntLineCow.outfit2 then return true end
...
function HuntLineCow.getSpawnRandomZedInfo(fit)
...
function HuntLineCow.doDespawn(zed)
    if zed:isAlive() then
...
function HuntLineCow.doSpawn(sq, isDown, outfit)
    if sq then
        local x, y, z = sq:getX(), sq:getY(), sq:getZ()
        if isClient() then

            local fit, fChance = HuntLineCow.getSpawnRandomZedInfo()

            if outfit and outfit ~= '' then
                fit, fChance = HuntLineCow.getSpawnRandomZedInfo(outfit)
            end
            sendClientCommand('HuntLineCow', 'doSpawn', {x = x, y = y, z = z, count = 1, fit = fit, fChance = fChance, isDown = isDown})
        else
...
function HuntLineCow.doLocalSpawn(sq, outfit)
	local pl = getPlayer()
