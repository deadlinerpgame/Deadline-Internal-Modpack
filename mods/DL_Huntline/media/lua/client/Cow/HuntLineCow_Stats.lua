...
------------------------               ---------------------------
function HuntLineCow.doRoll(percent) if percent >= ZombRand(1, 101) then return true end return false end
-----------------------            ---------------------------


function HuntLineCow.TempMarker(sq, isGood)
...
function HuntLineCow.TicksSinceSeenZombie()
    local pl = getPlayer()
    return pl.TicksSinceSeenZombie >= 6000
end
function HuntLineCow.lastSeenZombieTime()
    local pl = getPlayer()
...
function HuntLineCow.isBiteReact(pl)
	local attacker = nil
...
function HuntLineCow.setStats(zed)
    if zed then
        ...
        if HuntLineCow.isCowAnimal(zed) then
            print("Test2")
            if zed:getModData()['CowAnimal_Init'] == nil then
                print("Test3")
                ...
                -----------------------            ---------------------------
      --[[           local tSpeed =  SandboxVars.HuntLineCow.TurnSpeed or 1.5
                if tSpeed then
                    HuntLineCow.setTurnSpeed(zed, tSpeed)
                end ]]
                -----------------------            ---------------------------
                zed:getModData()['Animal_fType'] = HuntLineCow.getCorpseFtype(zed)
                zed:setVariable('isCowAnimal', 'true')
                -----------------------            ---------------------------

                HuntLineCow.setTag(zed)
                ...
                zed:getModData()['CowAnimal_Init'] = true
            end
