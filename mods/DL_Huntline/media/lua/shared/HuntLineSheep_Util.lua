...
function HuntLineSheep.findzedID(int)
...
function HuntLineSheep.normalizeTo100(x, min, max)
    x = math.max(min, math.min(x, max))
    return ((x - min) / (max - min)) * 100
end

-----------------------            ---------------------------
function HuntLineSheep.checkDist(pl, zed)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return math.floor(dist)
end

function HuntLineSheep.isWithinRange(pl, zed, range)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return dist <= range
end

function HuntLineSheep.isClosestPl(pl, zed)
	if not HuntLineSheep.isSheepAnimal(zed) then return end
	local plDist = HuntLineSheep.checkDist(pl, zed)
