...
function HuntLineCow.findzedID(int)
...
function HuntLineCow.normalizeTo100(x, min, max)
    x = math.max(min, math.min(x, max))
    return ((x - min) / (max - min)) * 100
end

-----------------------            ---------------------------
function HuntLineCow.checkDist(pl, zed)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return math.floor(dist)
end

function HuntLineCow.isWithinRange(pl, zed, range)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return dist <= range
end

function HuntLineCow.isClosestPl(pl, zed)
	if not HuntLineCow.isCowAnimal(zed) then return end
	local plDist = HuntLineCow.checkDist(pl, zed)
