...
function HuntLineRat.findzedID(int)
...
function HuntLineRat.normalizeTo100(x, min, max)
    x = math.max(min, math.min(x, max))
    return ((x - min) / (max - min)) * 100
end

-----------------------            ---------------------------
function HuntLineRat.checkDist(pl, zed)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return math.floor(dist)
end

function HuntLineRat.isWithinRange(pl, zed, range)
	local dist = pl:DistTo(zed:getX(), zed:getY())
    return dist <= range
end

function HuntLineRat.isClosestPl(pl, zed)
	if not HuntLineRat.isRatAnimal(zed) then return end
	local plDist = HuntLineRat.checkDist(pl, zed)
